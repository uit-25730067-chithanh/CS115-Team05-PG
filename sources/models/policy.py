...
class PolicyNetwork(nn.Module):
    ...
    Biểu diễn hàm policy: pi(a | s, theta)
    """
    ...
    def select_action(self, state, device="cpu"):
        """
        Chọn hành động dựa trên xác suất p(a|s).
        Returns:
            action: Hành động (số nguyên)
            log_prob: Log-xác suất của hành động được chọn log(pi(a|s))
        """
        # Chuyển numpy state thành torch tensor
