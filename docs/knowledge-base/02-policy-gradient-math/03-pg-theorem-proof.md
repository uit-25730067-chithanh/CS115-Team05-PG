...
## 3. Chứng minh Bổ đề Nhân quả (Causality)

Trong REINFORCE, chúng ta thay thế tổng phần thưởng cả trajectory $R(\tau)$ bằng phần thưởng tương lai (reward-to-go) $G_t = \sum_{k=t}^T r_k$. Tại sao ta có thể bỏ qua các phần thưởng trong quá khứ ($r_{k < t}$)?

**Chứng minh:**
Ta cần chứng minh rằng phần thưởng quá khứ không đóng góp vào gradient kỳ vọng:
$$\mathbb{E}_{\tau \sim \pi} \left[ \nabla_\theta \ln \pi(a_t|s_t) \cdot r_k \right] = 0 \text{ với } k < t$$

Sử dụng luật kỳ vọng lặp (Law of Iterated Expectations):
$$\mathbb{E}_{s_t, a_t} \left[ \nabla_\theta \ln \pi(a_t|s_t) \cdot r_k \right] = \mathbb{E}_{s_t} \left[ r_k \cdot \mathbb{E}_{a_t \sim \pi} [\nabla_\theta \ln \pi(a_t|s_t) | s_t] \right]$$
...
Vì số hạng này luôn bằng 0, các phần thưởng quá khứ $r_k$ không làm thay đổi kỳ vọng của gradient, nhưng việc loại bỏ chúng giúp giảm phương sai (variance) đáng kể.
