...
├── outputs/    # Kết quả final đã chọn lọc; run mới sinh ra được ignore
...
15. **Output policy**: Các run timestamp là generated artifacts. Repo public chỉ giữ bộ kết quả chọn lọc dưới `outputs/final/`; các run mới sinh ra trong quá trình dev được ignore.
